...
def generate_channel_gains(n):
    """Generate random channel gains using a normal distribution between 0 and 1."""
    gains = np.random.normal(loc=0.5, scale=0.2, size=n)  # Center around 0.5 with some spread
...
def kmeans_pairing(h):
    """K-Means based user pairing."""
    n = len(h)
    h_reshaped = np.array(h).reshape(-1, 1)
    kmeans = KMeans(n_clusters=n // 2, random_state=0, n_init=10)
    labels = kmeans.fit_predict(h_reshaped)
    cluster_pairs = {}
    
    for idx, label in enumerate(labels):
        if label in cluster_pairs:
            cluster_pairs[label].append(h[idx])
        else:
            cluster_pairs[label] = [h[idx]]
    
    pairs = [tuple(sorted(v)) for v in cluster_pairs.values() if len(v) == 2]
    return pairs
...
# Measure execution time and apply all methods
start = time.time()
...
kmeans_avg_diff, kmeans_std, kmeans_failures, kmeans_pair_list, kmeans_differences = evaluate_pairing(kmeans_pairs, h_threshold)
...
print(f"K-Means\t{kmeans_avg_diff:.4f}\t{kmeans_std:.4f}\t{kmeans_failures}\t{kmeans_time:.6f} sec")
...
print("\nK-Means Method Pairs and Differences:")
for pair, diff in zip(kmeans_pair_list, kmeans_differences):
    print(f"Pair: {pair}, Difference: {diff:.4f}")

# Visualization
x_labels = ['My Method', 'Your Method', 'K-Means']
values = [my_avg_diff, your_avg_diff, kmeans_avg_diff]
std_dev = [my_std, your_std, kmeans_std]
failures = [my_failures, your_failures, kmeans_failures]

fig, ax1 = plt.subplots()
ax2 = ax1.twinx()
ax1.bar(x_labels, values, color=['blue', 'green', 'purple'], alpha=0.6, label='Avg Difference')
